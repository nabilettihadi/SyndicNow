...
public class ProprietaireController {
    ...
    public ResponseEntity<ProprietaireResponseDTO> createProprietaire(@Valid @RequestBody ProprietaireCreateDTO dto) {
        log.info("Creating new owner with data: {}", dto);
        ProprietaireResponseDTO response = proprietaireService.createProprietaire(dto);
    ...
    public ResponseEntity<ProprietaireResponseDTO> updateProprietaire(
            @Parameter(description = "ID of the owner to update") @PathVariable Long id,
            @Valid @RequestBody ProprietaireUpdateDTO dto) {
        log.info("Updating owner with ID: {} with data: {}", id, dto);
        ProprietaireResponseDTO response = proprietaireService.updateProprietaire(id, dto);
    ...
    @ApiResponse(responseCode = "404", description = "Owner not found")
    public ResponseEntity<ProprietaireResponseDTO> getProprietaireById(
            @Parameter(description = "ID of the owner to retrieve") @PathVariable Long id) {
        log.debug("Fetching owner with ID: {}", id);
        ProprietaireResponseDTO response = proprietaireService.getProprietaireById(id);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<List<ProprietaireResponseDTO>> getAllProprietaires() {
        log.debug("Fetching all owners");
        List<ProprietaireResponseDTO> response = proprietaireService.getAllProprietaires();
        return ResponseEntity.ok(response);
    }
